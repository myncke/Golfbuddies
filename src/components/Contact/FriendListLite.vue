        ...
        <v-list two-line style="height: 25vh; overflow:scroll;" class="white">
          <template v-for="model in friendUserModels">
            <v-list-tile avatar v-bind:key="model.user.key">
              ...
              <v-list-tile-action>
                <v-tooltip right>
                  <v-btn icon ripple @click="openConversation(model)" slot="activator">
                    <v-icon color="grey lighten-1">comment</v-icon>
                  </v-btn>
                  <span>Chat with {{model.user.nickname}}</span>
                </v-tooltip>

                <v-tooltip right>
                  <v-btn icon ripple @click="closeFriend(model.friendship)" slot="activator">
